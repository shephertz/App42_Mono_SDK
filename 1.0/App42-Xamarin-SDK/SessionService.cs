...
{
    /// <summary>
    /// The Session Manager manages user sessions on the server side. It is a persistent Session Manager. It allows to save attributes in 
    /// the session and retrieve them. This Session Manager is especially useful for Mobile/Device Apps which want to do session management.
    /// </summary>
    /// <see cref="SessionService">Session</see>
    public class SessionService
    {
        ...
        /// <returns>Session object containing the session id of the created session.This id has to be used for storing or retrieving attributes.
        /// </returns>
        ...
        /// <summary>
        /// Create User Session based on the isCreate boolean parameter. If isCreate is true and there is an existing session for the user,
        /// the existing session is returned. If there is no existing session for the user a new one is created.If isCreate is false and
        /// there is an existing session, the existing session is returned if there is no existing session new one is not created.
        /// </summary>
        /// <param name="uName">UserName for which the session has to be created.</param>
        ...
        /// <returns>Session object containing the session id of the created session. This id has to be used for storing or retrieving attributes.</returns>
        /// <exception>App42Exception</exception>
        public Session GetSession(String uName, Boolean isCreate)
        {
            String response = null;
            Session sessionObj = null;
            Util.ThrowExceptionIfNullOrBlank(uName, "User Name");
        ...
        /// <returns>App42Response if invalidated successfully</returns>
        /// <exception>App42Exception</exception>
        ...
        /// <returns>Session object containing the attribute and value which is stored.</returns>
        /// <exception>App42Exception</exception>
        public Session SetAttribute(String sessionId, String attributeName,
        ...
        /// <returns>Session object containing the attribute and value which is stored for the session id and attribute name.</returns>
        /// <exception>App42Exception</exception>
        public Session GetAttribute(String sessionId, String attributeName)
        {
            String response = null;
            Session sessionObj = null;
            Util.ThrowExceptionIfNullOrBlank(sessionId, "Session ID");
        ...
        /// </summary>
        /// <param name="sessionId">The session id for which the attribute has to be fetched.</param>
        /// <returns>Session object containing all the attributes and values which are stored for the session id.</returns>
        /// <exception>App42Exception</exception>
        public Session GetAllAttributes(String sessionId)
        {
            String response = null;
            Session sessionObj = null;
            Util.ThrowExceptionIfNullOrBlank(sessionId, "Session Id");
